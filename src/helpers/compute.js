export function compute({years = 1, startAmount = 0, contributionPerAmount = 0, contributionFrequencyPerYear = 0, compoundingPerYear = 1 , percentage = 0 }) {
	...
	for (let year = 1; year <= years; year++) { 
		for ( let cycle = 1; cycle <= cycles; cycle++) {
			if(cycle % rCompounding === 0 && compoundingPerYear !== 0) {
				result = result + computeNewInterest(percentage,result, compoundingPerYear)
			}
			if(cycle % rContribution === 0) {
				result = result + contributionPerAmount
			}
		}
	}
	return result;
